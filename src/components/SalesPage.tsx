...
async function getData(date: String, id: Number) {
	return JSON.stringify(await fetch(`http://localhost:3000/sales/${date}/${id}`, { method: 'get' })
		.then((response: { text: () => any; }) => response.text())
		.then((data: any) => {
			return data;
		}));
...
export function SalesGraph(props: { id: number, date: string }) {
	...
	React.useEffect(() => {
		const Data = getData(props.date, props.id).catch((reason) => {
			window.alert(`An error occured when I tried to get the sales data. 
Please ensure the server is running
For advanced users: ${reason}`); return [{ 0: 0 }];
		}).then((data) => setSalesData({ x: data, y: props.date }));
	}, [props.date]);
